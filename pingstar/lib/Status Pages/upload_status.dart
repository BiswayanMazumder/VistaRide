...
class StatusUploading extends StatefulWidget {
  final File? image; // Image file to upload
  final String otherUID; // Receiver's UID

  const StatusUploading({Key? key, required this.image, required this.otherUID}) : super(key: key);

  @override
  State<StatusUploading> createState() => _StatusUploadingState();
}

class _StatusUploadingState extends State<StatusUploading> {
  ...
  Future<void> sendImageMessage(String imageUrl) async {
    try {
      ...
      final docRef = await FirebaseFirestore.instance.collection('Users Status').doc(_auth.currentUser!.uid).set(
          {
            'Image URL':imageURL,
            'Upload Time':FieldValue.serverTimestamp(),
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Container(
        ...
        child: Row(
          ...
          children: [
          issending? const CircularProgressIndicator(
